...
const IntercityRouteMap = dynamic(
  () => import('@/components/IntercityRouteMap').then((m) => m.IntercityRouteMap),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-48 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center text-xs font-bold text-slate-400 animate-pulse">
...
const POPULAR_INTERCITY_CITIES = [
  'Turbat',
  'Gwadar',
  'Pasni',
  'Jiwani',
...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col lg:flex-row">
      ...
      {/* INTERCITY ROUTE MODAL WITH LIVE GOOGLE MAPS CORRIDOR */}
      {routeModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 max-w-2xl w-full max-h-[92vh] overflow-y-auto space-y-4 shadow-2xl animate-scaleIn">
            <div className="flex items-center justify-between pb-2.5 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-black text-slate-900 text-sm sm:text-base">Configure Intercity Highway Route & Fares</h3>
                  <p className="text-[11px] text-slate-500">Pick origin & destination, inspect live Google route, and auto-calculate distance</p>
                </div>
              </div>
              <button 
            ...
            <form onSubmit={handleSaveRouteSubmit} className="space-y-3.5 text-xs">
                ...
                <IntercityRouteMap
                  ...
                  onRouteCalculated={(km, mins) => {
                    ...
                    setEditingRoute((prev) => ({
                      ...
                      estimated_duration: `${hours} Hours`,
                    }));
                  }}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Calculated Distance (KM)</label>
                  <input
                    type="number"
                    required
                    value={editingRoute.estimated_distance_km || 100}
                    onChange={(e) => setEditingRoute({ ...editingRoute, estimated_distance_km: Number(e.target.value) })}
                    className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 font-bold text-emerald-900"
                  />
                </div>
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Estimated Travel Duration</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200">
                <div>
                  ...
                  >
                    <option value="fixed">Fixed Flat Route Rate</option>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <input
                    type="number"
                    value={editingRoute.car_economy_fare ?? 0}
                    onChange={(e) => setEditingRoute({ ...editingRoute, car_economy_fare: Number(e.target.value) })}
                    className="w-full bg-white border border-slate-300 rounded-xl px-2 py-1.5 font-bold text-slate-900"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={editingRoute.car_comfort_fare ?? 0}
                    onChange={(e) => setEditingRoute({ ...editingRoute, car_comfort_fare: Number(e.target.value) })}
                    className="w-full bg-white border border-slate-300 rounded-xl px-2 py-1.5 font-bold text-teal-700"
                  />
                </div>
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Parcel Cargo (PKR)</label>
                  <input
                    type="number"
                    value={editingRoute.delivery_parcel_fare ?? 0}
                    onChange={(e) => setEditingRoute({ ...editingRoute, delivery_parcel_fare: Number(e.target.value) })}
                    className="w-full bg-white border border-slate-300 rounded-xl px-2 py-1.5 font-bold text-amber-700"
                  />
                </div>
              </div>
              ...
              <div className="pt-2 flex gap-2">
                <button
                  type="button"
                  onClick={() => setRouteModalOpen(false)}
                  className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2.5 rounded-xl cursor-pointer transition"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isSaving}
                  className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-black py-2.5 rounded-xl cursor-pointer shadow-sm transition"
                >
                  {isSaving ? 'Saving Route...' : 'Save Highway Route'}
                </button>
              </div>
            </form>
          </div>
